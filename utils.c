...
void printf_TPM_REQUEST(BYTE *buf) {
    ...
    switch (ordinal) {
        ...
        case TPM_ORD_Sign: {
            ...
            tpm_unmarshal_TPM_KEY_HANDLE(&ptr, &length, &keyHandle);
            tpm_unmarshal_UINT32(&ptr, &length, &areaToSignSize);
            printf("TPM_KEY_HANDLE keyHandle: 0x%x\n", keyHandle);
            printf("UINT32 areaToSignSize :%x\n", areaToSignSize);
            printf_buf("BYTE areaToSign:", ptr, areaToSignSize);
            ptr += areaToSignSize;
            length -= areaToSignSize;
            break;
        }
    ...
    switch (ordinal) {
        ...
        case TPM_ORD_CreateWrapKey: {
            if (res == TPM_SUCCESS) {
                ...
                printf_buf("Public portion of WrappedKey:", wrappedKey.pubKey.key, wrappedKey.pubKey.keyLength);
        ...
        case TPM_ORD_Sign: {
            if (res == TPM_SUCCESS) {
                ...
                printf_buf("BYTE sig", ptr, sigSize);
                ptr += sigSize;
                length -= sigSize;
            }
            break;
        ...
        case TPM_ORD_MakeIdentity: {
            if (res == TPM_SUCCESS) {
                ...
                printf_buf("Public portion of idKey:", idKey.pubKey.key, idKey.pubKey.keyLength);
