...
{
  if (tpm_marshal_TPM_STRUCTURE_TAG(ptr, length, v->tag)
      || tpm_marshal_UINT32(ptr, length, v->currentTicks)
...
{
  if (tpm_unmarshal_TPM_STRUCTURE_TAG(ptr, length, &v->tag)
      || tpm_unmarshal_UINT32(ptr, length, &v->currentTicks)
...
{
    /* v->size is the size of the whole TPM_REQUEST
     * v->paramSize is the size of the request parameters
     */
